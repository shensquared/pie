...
# list = "/Users/shenshen/Codes/3900/scripts/sheets/" + term + "/classlst.xls"
list = "/Users/shenshen/Codes/3900/scripts/sheets/" + term + "/" + filenmae
...
with open(list) as f:
    for (idx, line) in enumerate(f):
        if filenmae == "prereg.xls" and not line.startswith('"9'):
            continue
        fields = line.split("\t")
        if len(fields) <= 4:
            continue
...
if dept_or_year == "dept":
    path = ["reg_or_can_list", "dept_list", "detail_major_list", "year_list"]
    if term == "spring23":
        path = path[1:]
    ...
    fig = px.sunburst(
        ...
        title="6.390 Enrollment - " + term + " - by department",
    )
else:
    ...
    fig = px.sunburst(
        ...
        color_continuous_scale="RdBu",
        # color_continuous_midpoint=np.average(df["reg_or_can_list"]),
        title="6.390 Enrollment - " + term + " - by student-year",
    )
